...
check_dependencies() {
    ...
    local tools=("gcc" "g++" "make" "flex" "bison" "gawk" "curl" "tar" "xz")
    ...
    # Check for makeinfo (from texinfo package) - may be in /bin or /usr/bin
    if ! command -v makeinfo &>/dev/null && ! type makeinfo &>/dev/null && \
       ! [[ -x /bin/makeinfo ]] && ! [[ -x /usr/bin/makeinfo ]]; then
        missing+=("texinfo")
    fi

    # Check for required libraries using dpkg (most reliable on Debian/Ubuntu)
    # Fall back to header file detection if dpkg is not available
    local lib_pkgs=("libgmp-dev" "libmpfr-dev" "libmpc-dev")
    if command -v dpkg-query &>/dev/null; then
        ...
        # Fallback: check header files in common locations
        local gmp_found=false
        for hdr in /usr/include/gmp.h /usr/include/x86_64-linux-gnu/gmp.h \
                   /usr/local/include/gmp.h; do
            [[ -f "$hdr" ]] && gmp_found=true && break
        done
        $gmp_found || missing+=("libgmp-dev")

        local mpfr_found=false
        for hdr in /usr/include/mpfr.h /usr/include/x86_64-linux-gnu/mpfr.h \
                   /usr/local/include/mpfr.h; do
            [[ -f "$hdr" ]] && mpfr_found=true && break
        done
        $mpfr_found || missing+=("libmpfr-dev")

        local mpc_found=false
        for hdr in /usr/include/mpc.h /usr/include/x86_64-linux-gnu/mpc.h \
                   /usr/local/include/mpc.h; do
            [[ -f "$hdr" ]] && mpc_found=true && break
        done
        $mpc_found || missing+=("libmpc-dev")
    fi
