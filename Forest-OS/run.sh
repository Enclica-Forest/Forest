...
#
# Configuration is loaded from build-config.mk (generated by ./conf.sh).
# CLI arguments override config values. Run ./conf.sh --menuconfig to change config.
...
# Exports ARCH, BOOT_MODE, BUILD_TYPE, QEMU_*, ENABLE_*, etc.
if [ -f build-config.mk ]; then
    eval "$(./tools/config-env.sh)" 2>/dev/null || true
fi

# Derive sound device from ENABLE_AUDIO + ENABLE_SOUND_* config vars.
# Returns "none" if audio disabled, otherwise the first enabled sound device.
derive_sound_from_config() {
    if [ "${ENABLE_AUDIO:-no}" != "yes" ]; then
        echo "none"
        return 0
    fi
    if [ "${ENABLE_SOUND_SB16:-no}" = "yes" ]; then
        echo "sb16"
    elif [ "${ENABLE_SOUND_AC97:-no}" = "yes" ]; then
        echo "ac97"
    elif [ "${ENABLE_SOUND_HDA:-no}" = "yes" ]; then
        echo "hda"
    elif [ "${ENABLE_SOUND_ENSONIQ:-no}" = "yes" ]; then
        echo "es1370"
    elif [ "${ENABLE_SOUND_OPL3:-no}" = "yes" ]; then
        echo "adlib"
    else
        echo "none"
    fi
...
# Config-derived defaults (CLI args below still override these)
ARCH="${ARCH:-64}"
BOOT_MODE="${BOOT_MODE:-uefi}"
DEFAULT_ARCH="$ARCH"
DEFAULT_MEMORY="${QEMU_MEMORY:-512}"
DEFAULT_TIMEOUT="0"
DEFAULT_ISO=""
DEFAULT_MODE="graphics"
DEFAULT_SOUND="$(derive_sound_from_config)"
...
show_usage() {
    ...
    echo ""
    echo "Configuration is loaded from build-config.mk (managed by ./conf.sh)."
    echo "CLI options override config values."
    echo ""
    echo "Quick Options:"
    echo "  -h, --help           Show this help message"
    echo "      --uefi            Use UEFI boot (config default: ${BOOT_MODE})"
    echo "      --bios            Use BIOS boot"
    echo "  -a, --arch 32|64     Set architecture (config default: ${DEFAULT_ARCH})"
    echo "  -m, --memory MB      Set memory size in MB (config default: ${DEFAULT_MEMORY})"
    ...
    echo "  --build              Build before running (respects ./conf.sh config)"
    echo "  --sound DEVICE       Set sound device (config default: ${DEFAULT_SOUND})"
    echo "  --dry-run             Show QEMU command without executing"
    echo ""
    echo "Sound devices: sb16, ac97, hda, es1370, adlib, gus, none"
    echo ""
    echo "Examples for Canopy testing:"
    echo "  $0                           # Test Canopy with config defaults"
    echo "  $0 --bios                    # Test Canopy with BIOS boot"
...
# Function to build the OS (respects build-config.mk; no ARCH/BOOT_MODE override)
build_os() {
    print_info "Building Forest-OS (using ./conf.sh config)..."
    if make; then
...
run_qemu() {
    ...
    # KVM: respect QEMU_ENABLE_KVM from config (default yes), still check /dev/kvm
    if [ "${QEMU_ENABLE_KVM:-yes}" = "yes" ] && [ -r /dev/kvm ]; then
        qemu_cmd="$qemu_cmd -enable-kvm"
        print_info "  KVM acceleration: enabled"
    else
        print_warning "  KVM acceleration: disabled (not available or disabled in config)"
    fi

    # USB: respect QEMU_USB from config (default yes)
    if [ "${QEMU_USB:-yes}" = "yes" ]; then
        qemu_cmd="$qemu_cmd -device usb-ehci"
        print_info "  Input: USB mouse and keyboard (for Canopy)"
    else
        print_info "  USB: disabled in config"
    fi

    # Network: respect QEMU_NETWORK and ENABLE_NETWORKING from config
    if [ "${QEMU_NETWORK:-yes}" = "yes" ] && [ "${ENABLE_NETWORKING:-no}" = "yes" ]; then
        qemu_cmd="$qemu_cmd -netdev user,id=net0 -device rtl8139,netdev=net0"
        print_info "  Network: User-mode networking enabled"
    else
        print_info "  Network: disabled in config"
    fi

    # Sound configuration (sound_device derived from ENABLE_SOUND_* or set by --sound)
    local sound_config
...
# Initialize variables from config-derived defaults
...
# Parse command line arguments (override config-derived defaults)
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            show_usage
            ...
            if [ "$BOOT_MODE" = "uefi" ]; then
                ...
                if [ -z "$iso_file" ]; then
                    ...
                    if [ -z "$iso_file" ]; then
                        ...
                        print_info "Run ./conf.sh --menuconfig to configure, then make to build."
                ...
                print_info "Run ./conf.sh --menuconfig to configure, then make iso to build."
                print_info "Available files in dist/:"
                ls -la dist/*.{iso,img} 2>/dev/null || echo "  No images found"
                exit 1
            fi
