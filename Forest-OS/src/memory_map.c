...
/* Renamed from memory_get_usable_kb to avoid clashing with the authoritative
 * implementation in memory.c. This memory_map.c subsystem's memory_map_init()
 * is never called, so memory_map_initialized stays false and this returned 0 —
 * and because the kernel links with --allow-multiple-definition, this dead copy
 * was shadowing memory.c's correct value for kernel.c / mm_buddy.c / vmm.c,
 * making the kernel wrongly report 0 KB of RAM. */
uint32 memory_map_get_usable_kb(void) {
    if (!memory_map_initialized) {
        return 0;
    }
