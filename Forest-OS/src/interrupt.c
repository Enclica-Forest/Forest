...
// Landing pad for a task whose context faulted with an invalid opcode.
// handle_invalid_opcode() redirects the faulted context's EIP here instead
// of letting it resume mid-corruption. We just halt-loop: interrupts stay
// enabled, so the timer ISR keeps firing and task_schedule() (called from
// there, not reentrantly from the fault handler) will see this task's
// pending SIGKILL and destroy it through the normal, already-tested
// scheduling path, switching away to something healthy.
static void invalid_opcode_park_and_die(void) {
    for (;;) {
...
//
// This used to "handle" every invalid opcode by skipping 1-9 bytes and
// resuming execution as if nothing happened, under the label "emulation" -
// but it never actually emulated any instruction semantics, it just walked
// forward through whatever memory was there. An invalid opcode always means
// the executing context (registers, return address, or the code itself) is
// already corrupted; skipping bytes and resuming just executes more
// garbage, which is a near-guaranteed unrecoverable crash-loop (each retry
// almost always lands on more invalid bytes) rather than a real recovery.
//
// The actually safe options are: (a) if a specific, non-critical task was
// executing, isolate the damage to just that task - kill it and park its
// context in a harmless halt loop, letting the rest of the system keep
// running; (b) otherwise (no task context, or it's a protected/critical
// task like the boot thread or idle task) fall through to a real panic
// with full diagnostics instead of pretending to recover.
static bool handle_invalid_opcode(struct interrupt_frame* frame) {
    ...
    if (current_task && !current_task->is_protected) {
        debuglog(DEBUG_ERROR,
                 "[KERNEL] Invalid opcode in task %u ('%s') at eip=0x%x - killing task, not the system\n",
                 current_task->id, current_task->name, (uint32)FRAME_IP(frame));
        task_kill(current_task->id);
        FRAME_IP(frame) = (uintptr_t)invalid_opcode_park_and_die;
        return true;
    }

    // No task to isolate the damage to (or it's a protected/critical one) -
    // hand off to the unified panic renderer instead of faking a recovery.
    uintptr_t cr2 = 0;
#if ARCH_64BIT
    __asm__ __volatile__("mov %%cr2, %0" : "=r"(cr2));
#endif

    panic_preload_fault_info((uint32)cr2, (uint32)*instruction,
                             (uint32)FRAME_IP(frame), (uint32)frame->cs,
                             (uint32)FRAME_FLAGS(frame));
    kernel_panic_annotated("INVALID OPCODE EXCEPTION", __FILE__, __LINE__, __func__);
...
static void default_exception_handler(int int_no, struct interrupt_frame* frame, unsigned int error_code) {
    ...
    if (debuglog_is_ready()) {
        ...
        // Avoid deep unwinding in this path; stack walking can re-fault on a damaged stack.
        debuglog_write("Stack trace: skipped in default_exception_handler (panic path handles capture)\n");
    }
    ...
    // Build a concise panic headline and hand off to hardened panic flow.
    // Do not continue with local crash-screen loops here; those can stall mid-render
    // while other subsystems are still active.
    char title[64];
    char message[128];
    ...
    // Attach raw fault metadata so PanicUI can render registers/fault address.
    panic_preload_fault_info((uint32)cr2, (uint32)error_code,
                             (uint32)FRAME_IP(frame), (uint32)frame->cs,
                             (uint32)FRAME_FLAGS(frame));

    // Convert default exception path into the unified panic renderer.
    // This avoids partial exception dumps followed by a silent halt.
    kernel_panic_annotated(title, __FILE__, __LINE__, __func__);
}
