...
static volatile uint32 g_panic_render_stage = 0;
...
static void draw_bsod_header(const panic_context_t* ctx) {
    ...
    strcpy(header_text, "Fern v1.1 - A CRITICAL SYSTEM ERROR HAS OCCURRED");
...
static void render_system_info_page(const panic_context_t* ctx, int start_y) {
    ...
    panic_draw_text("Operating System: Fern v2.0", 30, y, FG_WHITE);
    y += FONT_HEIGHT;
...
static uint32_t panic_diag_hash(const panic_context_t* ctx) {
    uint32_t h = 2166136261u;
    if (!ctx) {
        return h;
    }

    for (size_t i = 0; i < sizeof(ctx->message) && ctx->message[i]; i++) {
        h ^= (uint8_t)ctx->message[i];
        h *= 16777619u;
    }
    for (size_t i = 0; i < sizeof(ctx->file) && ctx->file[i]; i++) {
        h ^= (uint8_t)ctx->file[i];
        h *= 16777619u;
    }
    h ^= ctx->line;
    h *= 16777619u;
...
static void panic_draw_diag_qr_direct(uint8_t* fb, uint32_t width, uint32_t height,
                                      uint32_t pitch, uint32_t bpp,
                                      int32_t x, int32_t y, uint32_t hash) {
    const int size = 21;
    const int quiet = 2;
    const int module = 4;
    const int full = (size + quiet * 2) * module;

    char token[20] = "TOKEN:00000000";
    static const char hex[] = "0123456789ABCDEF";
    for (int i = 0; i < 8; i++) {
        token[6 + i] = hex[(hash >> ((7 - i) * 4)) & 0xF];
    }

    memset(qr_code_buffer, 0, sizeof(qr_code_buffer));
    memset(qr_temp_buffer, 0, sizeof(qr_temp_buffer));
    ...
    for (int py = 0; py < full; py++) {
        for (int px = 0; px < full; px++) {
            panic_draw_pixel_direct(fb, width, height, pitch, bpp, x + px, y + py, 255, 255, 255);
        }
    }

    for (int my = 0; my < size && my < qr_size; my++) {
        for (int mx = 0; mx < size && mx < qr_size; mx++) {
            int on = qrcodegen_getModule(qr_code_buffer, mx, my);
            uint8_t c = on ? 0 : 255;
            int sx = x + (mx + quiet) * module;
            int sy = y + (my + quiet) * module;
            for (int dy = 0; dy < module; dy++) {
                for (int dx = 0; dx < module; dx++) {
                    if (sx + dx < x + full && sy + dy < y + full) {
                        panic_draw_pixel_direct(fb, width, height, pitch, bpp, sx + dx, sy + dy, c, c, c);
                    }
                }
            }
        }
...
static void panic_render_diag_qr_label(uint8_t* fb, uint32_t width, uint32_t height,
                                       uint32_t pitch, uint32_t bpp,
                                       int32_t x, int32_t y, uint32_t hash,
                                       uint8_t bg_r, uint8_t bg_g, uint8_t bg_b) {
    panic_draw_string_direct(fb, width, height, pitch, bpp, x, y,
                             "DIAG QR", 255, 255, 255, bg_r, bg_g, bg_b);
    ...
    panic_draw_string_direct(fb, width, height, pitch, bpp, x, y + 10,
                             token, 255, 255, 0, bg_r, bg_g, bg_b);
}

static bool panic_render_simple(const panic_context_t* ctx) {
    ...
    if (!device || !device->current_fb) {
        ...
        uint32_t hash = panic_diag_hash(ctx);
        int32_t qr_x = (int32_t)width - 120;
        int32_t qr_y = 40;
        panic_render_diag_qr_label(buffer, width, height, pitch, bpp, qr_x, qr_y - 18, hash, 139, 0, 0);
        panic_draw_diag_qr_direct(buffer, width, height, pitch, bpp, qr_x, qr_y, hash);
...
static bool panic_render_tty_minimal(const panic_context_t* ctx) {
    if (!tty_init()) {
        return false;
    }
    ...
    tty_clear();
    tty_write_ansi("KERNEL PANIC\n");
    tty_write_ansi("System halted.\n");

    if (ctx && ctx->message[0]) {
        tty_write_ansi("Message: ");
    ...
    if (ctx && ctx->file[0]) {
        char line_buf[16];
        tty_write_ansi("Location: ");
        tty_write_ansi(ctx->file);
        if (ctx->line > 0) {
            tty_write_ansi(":");
            format_decimal(ctx->line, line_buf);
            tty_write_ansi(line_buf);
        }
        tty_write_ansi("\n");
    }
    ...
    if (ctx) {
        char hex_buf[16];
        tty_write_ansi("EIP: ");
        format_hex32(ctx->cpu_state.eip, hex_buf);
        tty_write_ansi(hex_buf);
        tty_write_ansi("\n");
    }
...
static void panic_render_console_minimal(const panic_context_t* ctx) {
    console_init();
    clearScreen();
    print_colored("KERNEL PANIC\n", TEXT_ATTR_LIGHT_RED, TEXT_ATTR_BLACK);
    print_colored("System halted.\n", TEXT_ATTR_WHITE, TEXT_ATTR_BLACK);

    if (ctx && ctx->message[0]) {
        print_colored("Message: ", TEXT_ATTR_YELLOW, TEXT_ATTR_BLACK);
        print_colored(ctx->message, TEXT_ATTR_WHITE, TEXT_ATTR_BLACK);
        print_colored("\n", TEXT_ATTR_WHITE, TEXT_ATTR_BLACK);
    }
}

static void wait_for_panic_input(const panic_context_t* ctx) {
    (void)ctx;
    
    for (;;) {
        if (panic_mouse_enabled) {
            panic_poll_mouse_data();
            if (panic_mouse_state.left_button || panic_mouse_state.right_button) {
                return;
            }
        }
...
// Simple staged panic display with defensive fallbacks.
static void draw_simple_panic_screen(const panic_context_t* ctx) {
    g_panic_render_in_progress = true;

    init_panic_mouse_support();

    g_panic_render_stage = 1;
    if (panic_render_simple(ctx)) {
        wait_for_panic_input(ctx);
        return;
    }

    g_panic_render_stage = 2;
    if (panic_render_tty_minimal(ctx)) {
        wait_for_panic_input(ctx);
        return;
    }

    g_panic_render_stage = 3;
    panic_render_console_minimal(ctx);

    g_panic_render_stage = 4;
    (void)panic_try_render_panicui(ctx);
...
#ifndef NDEBUG
static bool panic_debug_has_terminator(const char* s, uint32 limit) {
    if (!s) {
        return false;
    }
...
// Tiny debug-only guardrail: validate panic context shape and emit a checksum
// before entering the final halt loop.
static void panic_debug_prehalt_selfcheck(const panic_context_t* ctx) {
    if (!debuglog_is_ready()) {
        return;
    }

    uint32 issues = 0;
    uint32 mix = 0x50414E49U; // "PANI"

    if (!ctx) {
        issues |= 0x0001U;
    } else {
        if (ctx->stack_frame_count > MAX_STACK_FRAMES) {
            issues |= 0x0002U;
        }
        if (ctx->manual_stack_count > MAX_STACK_FRAMES) {
            issues |= 0x0004U;
        }
        if (!panic_debug_has_terminator(ctx->message, sizeof(ctx->message))) {
            issues |= 0x0008U;
        }
        if (!panic_debug_has_terminator(ctx->file, sizeof(ctx->file))) {
            issues |= 0x0010U;
        }
        if (!panic_debug_has_terminator(ctx->function, sizeof(ctx->function))) {
            issues |= 0x0020U;
        }

        mix ^= ctx->cpu_state.eip;
        mix ^= ctx->cpu_state.esp;
        mix ^= ctx->cpu_state.cr2;
        mix ^= ctx->error_code;
        mix ^= ctx->line;

        for (uint32 i = 0; i < 16; i++) {
            mix = (mix << 5) ^ (mix >> 2) ^ (0x9E3779B9U + i);
        }
        asm volatile("" : "+r"(mix) :: "memory");
    }

    debuglog_write("[PANIC-SELFCHK] ");
    debuglog_write((issues == 0) ? "PASS issues=0x" : "FAIL issues=0x");
    debuglog_write_hex(issues);
    debuglog_write(" mix=0x");
    debuglog_write_hex(mix);
    debuglog_write("\n");
}
#else
static void panic_debug_prehalt_selfcheck(const panic_context_t* ctx) {
...
void kernel_panic_annotated(const char* message, const char* file, uint32 line, const char* func) {
    ...
    if (g_panic_in_progress) {
        ...
        panic_debug_prehalt_selfcheck(NULL);
        __asm__ volatile("cli; hlt");
        while(1) { __asm__ volatile("hlt"); }
    }
    g_panic_in_progress = true;
    // Freeze normal interrupt-driven activity immediately.
    ...
    panic_debug_prehalt_selfcheck(&ctx);
    asm volatile("cli");
    for(;;) {
        asm volatile("hlt");
    }
