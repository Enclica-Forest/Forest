...
void kmain(uint32 magic, uint32 mbi_addr) {
    ...
    *((uint16*)&test_elf_header[50]) = 0;
    ...
    // Enter the interactive session/login loop.
    // session_run() normally does not return; if it does (e.g. after logout),
    // we log the event, wait briefly, then restart the session so the user
    // is returned to the login prompt rather than leaving the system stuck.
    while (1) {
        session_run(autologin_root);
        if (debuglog_is_ready()) {
            debuglog_write("[KERNEL] Session ended, restarting login manager...\n");
        }
        autologin_root = false;
        if (g_silent_boot && g_multiboot_framebuffer) {
            splash_draw_background();
        }
