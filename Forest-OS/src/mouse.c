...
static void ps2_mouse_dispatch_input_event(const input_event_t* ev) {
    ...
    // Dispatch to input multiplexer (for desktop, etc.)
...
static void ps2_mouse_process_packet(void) {
    ...
    /* Update the graphical mouse cursor position in the window manager */
    if (window_manager_is_initialized()) {
        wm_update_cursor((int32_t)mouse_state.x, (int32_t)mouse_state.y);
    }
