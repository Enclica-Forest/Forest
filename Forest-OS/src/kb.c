...
static bool legacy_read_char(char* out_char) {
    ...
    // This legacy path doesn't track Ctrl state at all (no legacy_ctrl_*
    // variables exist), unlike the primary PS/2 IRQ path in ps2_keyboard.c -
    // passing false here preserves prior behavior rather than half-wiring
    // Ctrl support into a fallback path.
...
// Raw/non-canonical read used by sys_read(fd 0) when the controlling TTY has
// ICANON cleared (the shell's line editor, vi, htop, etc). Unlike readStr()
// -- which composes a whole cooked line, echoes, and only returns on Enter --
// this delivers bytes as they arrive, honoring VMIN:
//   vmin >= 1 : block until at least `vmin` bytes are available, then drain
//               whatever else is immediately buffered (up to `len`).
//   vmin == 0 : non-blocking; return only bytes already buffered (may be 0).
// No echo here: raw-mode readers echo themselves (or deliberately don't).
// VTIME is not finely honored (treated as "no timeout"); the common shell
// case is VMIN=1/VTIME=0, which this serves exactly.
uint32 keyboard_read_raw(char* dest, uint32 len, uint8 vmin) {
    if (!dest || len == 0) {
        return 0;
    }

    // Keyboard IRQs must be live so held/streamed scancodes reach the buffers
    // while we wait inside this syscall (mirrors readStr()).
    ...
    while (got < len) {
        ...
        if (keyboard_poll_char(&c)) {
            dest[got++] = c;
            if (vmin > 0 && got >= vmin) {
                // Minimum satisfied: sweep up any already-queued bytes so a
                // pasted/typematic burst arrives in one read(), then return.
                while (got < len && keyboard_poll_char(&c)) {
                    dest[got++] = c;
                }
                break;
            }
            ...
            if (vmin == 0) {
                break;                 // non-blocking: return what we have (maybe 0)
            }
            if (got >= vmin) {
                break;                 // already have the minimum
            }
            __asm__ __volatile__("hlt");  // sleep until the next interrupt
        }
    }
...
// Blocking single-char read used both by the main loop and by the arrow-key
// escape-sequence lookahead below, so the two stay in lockstep.
static bool kb_next_char_blocking(char* out) {
    while (1) {
        bool have = (current_driver_mode == KEYBOARD_DRIVER_PS2)
                        ? ps2_read_char(out)
                        : legacy_read_char(out);
        if (have) {
            return true;
        }
...
string readStr() {
    ...
    uint8 i = 0;       // number of characters in the line so far
    uint8 cursor = 0;  // insertion point within the line, 0 <= cursor <= i
    buffstr[0] = '\0';
    ...
    while (1) {
        ...
        // Arrow keys arrive here as the 3-byte ANSI/VT100 sequence
        // ESC '[' A/B/C/D (see keyboard_layout_emit_chars()'s arrow_map).
        // Left/Right move the cursor within the line instead of being
        // appended as three literal bytes; Up/Down are no-ops since there's
        // no command history to navigate.
        if ((uint8)ch == 0x1B) {
            char c2;
            if (!kb_next_char_blocking(&c2)) {
                continue;
            }
            if (c2 != '[') {
                continue;  // Unrecognized escape sequence: drop it
            }
            char c3;
            if (!kb_next_char_blocking(&c3)) {
                continue;
            }
            if (c3 == 'D') {
                if (cursor > 0) {
        ...
        if (ch == '\b' || (uint8)ch == 127) {
            if (cursor > 0) {
                bool at_end = (cursor == i);
                for (uint8 j = (uint8)(cursor - 1); j < (uint8)(i - 1); j++) {
                ...
                if (at_end) {
                    ...
                    print("\x1b[D");
                    for (uint8 j = cursor; j < i; j++) {
                        printch(buffstr[j]);
                    }
                    printch(' ');
                    uint8 back = (uint8)((i - cursor) + 1);
                    for (uint8 k = 0; k < back; k++) {
                        print("\x1b[D");
                    }
                }
            }
        ...
        if (i < KB_BUFFER_MAX - 1) {
            if (cursor == i) {
                // Common case: cursor already at the end, just append.
                printch(ch);
                buffstr[i++] = ch;
                cursor = i;
                buffstr[i] = '\0';
            } else {
                // Cursor is mid-line: insert and redraw the shifted tail.
                for (uint8 j = i; j > cursor; j--) {
                    buffstr[j] = buffstr[j - 1];
                }
                buffstr[cursor] = ch;
                i++;
