...
// Clear the minimal handler's re-entrancy guard. Must be called by any
// recovery action that kills the current task via task_terminate_current()
// (that call never returns, so the handler's own post-recovery decrement
// never executes for that path).
