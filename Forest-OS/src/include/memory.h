...
#define USER_STACK_SIZE         (256 * 1024) // 256KB default user stack (64 pages)
#define USER_STACK_SIZE_EMBEDDED (64 * 1024) // 64KB stack for embedded/low-mem mode
...
// Zero all currently free frames to remove stale RAM contents at boot
...
// Synchronize kernel-space PDE mappings into a task/page directory.
...
// Record the highest physical address (exclusive) that the kernel actually
// identity-mapped at boot (see memory.c's kernel_map_end). vmm.c uses this to
// know which physical addresses it may treat as directly-dereferenceable
// kernel virtual pointers (vmm_phys_pt_as_virt) versus which ones require
// going through vmm_temp_map_page(). Must be called once, after the boot
// identity map is established and before any page directories are cloned.
void vmm_set_identity_map_limit(uint32_t limit);

// Temporarily map a physical frame into the kernel's VMM temp-map window.
// Returns a kernel virtual pointer to access the frame. The returned address
// is only valid until the next call to vmm_temp_map_page (round-robin slots).
// Caller must call vmm_temp_unmap_page when done.
void* vmm_temp_map_page(uint32_t phys_addr);

// Unmap a page previously mapped by vmm_temp_map_page.
...
// Simple memory allocation functions (original Fern API)
...
// Graphics memory pool (pre-allocated for graphics operations)
