...
typedef struct {
    ...
    bool   is_pie;          // Position-independent (ET_DYN) executable
    uint32 load_bias;       // Offset applied to ET_DYN segments/entry
    char   interp[128];     // PT_INTERP program interpreter path (ldso)
    bool   has_interp;      // True if a PT_INTERP segment was present
    bool   has_dynamic;     // True if a PT_DYNAMIC segment was present
} elf_load_info_t;

/* Minimum free frames to keep in reserve after loading an ELF. */
#define ELF_MIN_FREE_FRAMES 64

/* Maximum number of program headers we will process per ELF. */
#define ELF_MAX_PHDR 256
