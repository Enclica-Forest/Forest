...
# (EXCLUDED_CSOURCES, EXCLUDED_*_SRCS, KERN_EXTRA_OBJS, USERSPACE_EXCLUDED,
# QEMU_OPTS) to gate sources when a feature is "no".
#
...
CSOURCES = $(filter-out \
    ...
    $(EXCLUDED_INPUT_SRCS) \
    $(EXCLUDED_INTERRUPT_SRCS) \
    $(EXCLUDED_CANOPY_SRCS),$(wildcard $(SRCDIR)/*.c)) $(SRCDIR)/symlink.c
...
# Canopy desktop environment (src/canopy/). Empty when CANOPY_DE_ENABLE=no.
CANOPY_SRCS = $(if $(filter no,$(CANOPY_DE_ENABLE)),,\
    $(filter-out $(EXCLUDED_CANOPY_SRCS),$(wildcard $(SRCDIR)/canopy/*.c)))
...
# CGDM display manager (top-level src/cgdm*.c). Empty when CANOPY_DM_ENABLE=no.
# cgdm_integration.c is dropped (it is in the base EXCLUDED_CSOURCES list).
CGDM_SRCS = $(if $(filter no,$(CANOPY_DM_ENABLE)),,$(wildcard $(SRCDIR)/cgdm*.c))
...
# ---------------------------------------------------------------------------
ALL_OBJECTS = $(COBJECTS) $(GRAPHICS_OBJECTS) $(PANICUI_OBJECTS) $(CANOPY_OBJECTS) \
