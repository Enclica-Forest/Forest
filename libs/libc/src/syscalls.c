...
int utimensat(int dirfd, const char *pathname, const struct timespec times[2], int flags) {
    return __syscall_ret(__syscall4(SYS_utimensat, dirfd, (syscall_arg_t)pathname, (syscall_arg_t)times, flags));
}
...
int seteuid(uid_t euid) {
    return __syscall_ret(__syscall2(SYS_setreuid, -1, euid));
}

int setegid(gid_t egid) {
    return __syscall_ret(__syscall2(SYS_setregid, -1, egid));
}
...
int getgroups(int size, gid_t list[]) {
    return __syscall_ret(__syscall2(SYS_getgroups, size, (syscall_arg_t)list));
}
...
int execvp(const char *file, char *const argv[]) {
    /* Try executing directly first */
    if (execve(file, argv, NULL) == 0) return 0;
    /* If it failed, try with /bin/ prefix */
    char path[256];
...
ssize_t pread(int fd, void *buf, size_t count, off_t offset) {
    return __syscall_ret(__syscall4(SYS_pread64, fd, (syscall_arg_t)buf, count, offset));
}

ssize_t pwrite(int fd, const void *buf, size_t count, off_t offset) {
    return __syscall_ret(__syscall4(SYS_pwrite64, fd, (syscall_arg_t)buf, count, offset));
}
...
/* Group operations (setgroups and ttyname stubs) */
int setgroups(size_t size, const gid_t *list) {
    return __syscall_ret(__syscall2(SYS_setgroups, size, (syscall_arg_t)list));
}

/* TTY operations */
char *ttyname(int fd) {
    static char buf[256];
    int ret = __syscall_ret(__syscall3(SYS_ioctl, fd, 0x5305 /* TIOCGDEV */, (syscall_arg_t)buf));
