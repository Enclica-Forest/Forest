...
void *memmove(void *dest, const void *src, size_t n) {
    ...
    /* Word-copy the aligned bulk of the region once the guard has already
     * bounded how many bytes are safe to touch; this is what made every
     * TTY scroll (screen.c scrollUp_locked, tty.c framebuffer scroll) a
     * visible byte-at-a-time "scan" instead of a near-instant bulk copy. */
    bool word_aligned = !(((uintptr_t)d | (uintptr_t)s | allowed) & (sizeof(unsigned long) - 1));

    if (d < s) {
        if (word_aligned) {
            unsigned long *ld = (unsigned long *)d;
            const unsigned long *ls = (const unsigned long *)s;
            size_t words = allowed / sizeof(unsigned long);
            while (words--) {
                *ld++ = *ls++;
            }
        } else {
        ...
        if (word_aligned) {
            unsigned long *ld = (unsigned long *)(d + allowed);
            const unsigned long *ls = (const unsigned long *)(s + allowed);
            size_t words = allowed / sizeof(unsigned long);
            while (words--) {
                *--ld = *--ls;
            }
