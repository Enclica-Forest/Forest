...
static uacpi_bool uacpi_ptr_is_canonical(const void *ptr)
{
#if UACPI_64BIT
    uacpi_uintptr v = (uacpi_uintptr)ptr;
    /* x86_64 canonical addresses: bits 63:48 must replicate bit 47 */
    uacpi_uintptr high = v >> 47;
...
uacpi_status uacpi_eval_hid(uacpi_namespace_node *node, uacpi_id_string **out_id)
{
    ...
    if (out_id != UACPI_NULL)
        *out_id = UACPI_NULL;

    if (node == UACPI_NULL || out_id == UACPI_NULL ||
        !uacpi_ptr_is_canonical(node) || !uacpi_ptr_is_canonical(out_id)) {
    ...
    if (hid_ret == UACPI_NULL || !uacpi_ptr_is_canonical(hid_ret)) {
    ...
    case UACPI_OBJECT_STRING: {
        ...
        if (uacpi_unlikely(buf == UACPI_NULL || !uacpi_ptr_is_canonical(buf) ||
                           buf->text == UACPI_NULL || !uacpi_ptr_is_canonical(buf->text))) {
    ...
    if (uacpi_unlikely(id == UACPI_NULL)) {
        return ret == UACPI_STATUS_OK ? UACPI_STATUS_OUT_OF_MEMORY : ret;
    }
