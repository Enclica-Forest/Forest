...
# =============================================================================
# LOAD BUILD CONFIGURATION (build-config.mk, generated by conf.sh --generate)
# =============================================================================
# Source config values as shell vars (ARCH, BOOT_MODE, BUILD_TYPE, VERBOSE,
# GENERATE_CHECKSUMS, ENABLE_TESTING, QEMU_*, ...). Fall back to defaults if
# build-config.mk is absent so the script still runs pre-configuration.
config_env_out="$("$SCRIPT_DIR/tools/config-env.sh" 2>/dev/null)" || {
    echo "WARN: no build-config.mk, using defaults"
    ARCH="${ARCH:-32}"
...
# Normalize yes/no (build-config.mk booleans) to true/false (internal convention).
normalize_bool() {
    case "$1" in
...
# Default configuration — derived from build-config.mk when available.
# The "build all variants" loop iterates over these; by default each list
# collapses to the single configured value, so a bare ./build-dist.sh builds
# only what build-config.mk specifies (matching `make`). Pass -a/-b/-t to
# expand the loop.
DEFAULT_ARCHITECTURES="${ARCH:-32}"
DEFAULT_BOOT_MODES="${BOOT_MODE:-bios}"
DEFAULT_BUILD_TYPES="${BUILD_TYPE:-debug}"
...
# Build options (honor build-config.mk; 0 parallel jobs => autodetect nproc)
PARALLEL_JOBS="${PARALLEL_JOBS:-0}"
if [ "$PARALLEL_JOBS" -eq 0 ]; then
    PARALLEL_JOBS="$(nproc)"
fi
VERBOSE="$(normalize_bool "${VERBOSE:-false}")"
CLEAN_BEFORE_BUILD="$(normalize_bool "${CLEAN_BEFORE_BUILD:-true}")"
GENERATE_CHECKSUMS="$(normalize_bool "${GENERATE_CHECKSUMS:-true}")"
COMPRESS_IMAGES="$(normalize_bool "${COMPRESS_IMAGES:-true}")"

# Advanced options
ENABLE_TESTING="$(normalize_bool "${ENABLE_TESTING:-false}")"
...
build_configuration() {
    ...
    # If this triple matches build-config.mk, let the Makefile read the config
    # directly (no CLI overrides) so config is the single source of truth.
    # When the loop is iterating over alternative variants (e.g. -a "32 64"),
    # pass ARCH/BOOT_MODE/BUILD_TYPE explicitly — make semantics make these
    # override build-config.mk.
    local build_cmd
    if [ "$arch" = "${ARCH:-$arch}" ] \
       && [ "$boot_mode" = "${BOOT_MODE:-$boot_mode}" ] \
       && [ "$build_type" = "${BUILD_TYPE:-$build_type}" ]; then
        build_cmd="make all -j$PARALLEL_JOBS"
    else
        build_cmd="make ARCH=$arch BOOT_MODE=$boot_mode BUILD_TYPE=$build_type all -j$PARALLEL_JOBS"
    fi
