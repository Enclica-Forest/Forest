...
// Renamed to avoid clashing with mm_cow_impl.c's real, actually-wired
// cow_get_stats()/cow_init() (differently-typed, differently-behaved --
// this file's Linux mm_struct/vma-style COW model is a separate, largely
// unused implementation; do_wp_page() is its only caller elsewhere).
// The unqualified names used to silently win the link over the real
// implementation, leaving cow_state uninitialized system-wide.
struct cow_stats *cow_get_stats_legacy_unused(void);
void cow_print_stats(void);
int cow_init_legacy_unused(void);
