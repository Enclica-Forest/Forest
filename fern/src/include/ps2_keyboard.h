...
typedef enum {
    ...
    // PS2_KEY_V auto-increments from PS2_KEY_C (0x4F) to 0x50. The next
    // explicit initializer MUST start at 0x51, not 0x50 - it previously said
    // 0x50 here, which collided PS2_KEY_B with PS2_KEY_V (identical enum
    // value), so every lookup keyed on PS2_KEY_V instead resolved whichever
    // table entry for PS2_KEY_B happened to be initialized last (visible as
    // pressing V producing 'b').
    PS2_KEY_B = 0x51, PS2_KEY_N, PS2_KEY_M, PS2_KEY_COMMA, PS2_KEY_PERIOD, PS2_KEY_SLASH,
