...
/* VGA text attribute: fg (bits 0-3) | bg (bits 4-6). Bit 7 is blink and must
 * NOT be set by the background value -- mask bg to 3 bits (0-7) to prevent
 * accidental blink when using color constants >= 0x08 as backgrounds. */
#define MAKE_TEXT_ATTR(fg, bg) (((fg) & 0x0F) | (((bg) & 0x07) << 4))
