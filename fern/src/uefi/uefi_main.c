/*
 * Fern UEFI Boot Entry Point
 *
 * This is the first C code executed by UEFI firmware after loading the
 * PE32+ image. It initialises the UEFI console, retrieves the memory map,
 * sets up a GOP framebuffer, locates ACPI tables, then hands off to the
 * kernel via kernel_main().
 */
 ...
 * If the build system doesn't define it, fall back to ms_abi. */
#ifndef EFIAPI
...
/* Include the canonical UEFI types from uefi_boot_services.h (via uefi.h).
 * We do NOT include uefi_console.h or uefi_acpi.h here because they pull in
 * uefi_runtime.h which defines conflicting typedefs for EFI_STATUS, EFI_GUID,
 * EFI_SYSTEM_TABLE, etc.  Instead we forward-declare the functions we need. */
#include "uefi.h"
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    /*
     * kernel_main() is expected to never return.
     * If it does, we tear down boot services and halt.
     */
    kernel_main();
