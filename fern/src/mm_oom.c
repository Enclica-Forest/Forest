...
#include "include/task.h" // task_for_each_oom_candidate(), task_kill() - real victim data/kill
#include <stddef.h>
#include <stdbool.h>
...
static bool oom_should_trigger(void)
{
    // reclaim_get_pressure() (mm_reclaim.c) is still backed by placeholder
    // zone stats and reads a constant fake value, not real memory state --
    // gating on it here would either always or never fire regardless of
    // actual pressure. pmm_get_free_frames() is real (backed by the PMM's
    // live bitmap accounting), so that's the one signal this checks against
    // until reclaim's zone stats are wired up for real.
    unsigned long free_pages = (unsigned long)pmm_get_free_frames();

    if (free_pages < OOM_THRESHOLD_PAGES) {
        return true;
    }
...
static int oom_kill_process(struct oom_task *task)
{
    ...
    // task_kill() sets a pending SIGKILL and, for a currently-blocked
    // victim, flips it straight to READY so the scheduler's fatal-signal
    // check (task_schedule()) picks it up on its next turn instead of
    // waiting indefinitely for it to wake on its own. Actual memory
    // reclamation happens asynchronously once the victim's task_exit()/
    // task_terminate_current() path runs and task_destroy() frees its
    // frames -- freed_pages here is this call's best estimate (current
    // rss+swap_usage), not a synchronously-confirmed count.
    task_kill((uint32)task->pid);

    return freed_pages;
}

/**
 * oom_visit_task - task_for_each_oom_candidate() callback: turn one real
 * task's fields into an oom_task list entry. Runs with no lock held (see
 * task_for_each_oom_candidate()'s contract), so this only ever reads the
 * by-value fields it was handed -- it never touches a task_t* directly.
 */
static void oom_visit_task(uint32 pid, const char *name, uint32 memory_used,
                            bool is_protected, bool is_init, void *ctx)
{
    ...
    task->total_vm = memory_used;
    task->rss = memory_used;
    task->swap_usage = 0; // Swap accounting isn't tracked per-task yet.
    task->is_kernel_thread = false;
    task->is_init = is_init;
    // Never-kill processes (init, or anything explicitly protected) get the
    // same "never kill" adjustment select_oom_victim() already special-cases
    // for OOM_SCORE_ADJ_MIN.
    task->oom_score_adj = is_protected ? OOM_SCORE_ADJ_MIN : 0;
...
static void oom_update_task_list(void)
{
    // Drop whatever the previous trigger populated -- without this, every
    // call appended instead of replacing, permanently leaking one oom_task
    // per past OOM event and re-scoring stale/duplicate entries alongside
    // real ones.
    struct oom_task *old, *tmp;
    list_for_each_entry_safe(old, tmp, &oom_control.task_list, list) {
        list_del(&old->list);
        kfree(old);
    }

    task_for_each_oom_candidate(oom_visit_task, NULL);
}
