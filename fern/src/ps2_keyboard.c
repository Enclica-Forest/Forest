...
static keyboard_layout_id_t ps2_keyboard_layout_from_config(void) {
    ...
    /* data points directly into the ramdisk buffer (not heap-allocated),
     * so it must NOT be freed. vfs_read_file() sets *data = f->data
     * which is a static ramdisk pointer. */
    return layout;
}
...
static uint16 keycode_to_linux[256] = {
    ...
    /* KEY_V (0x50, auto-continues from KEY_C=0x4F) through KEY_CAPS_LOCK
     * (0x5B, shifted +1 now that KEY_B correctly starts at 0x51 instead of
     * colliding with KEY_V at 0x50 - see ps2_keyboard.h). */
    [0x50] = 47,   /* KEY_V -> KEY_V */
    [0x51] = 48,   /* KEY_B -> KEY_B */
    [0x52] = 49,   /* KEY_N -> KEY_N */
    [0x53] = 50,   /* KEY_M -> KEY_M */
    [0x54] = 51,   /* KEY_COMMA -> KEY_COMMA */
    [0x55] = 52,   /* KEY_PERIOD -> KEY_DOT */
    [0x56] = 53,   /* KEY_SLASH -> KEY_SLASH */
    [0x57] = 54,   /* KEY_RIGHT_SHIFT -> KEY_RIGHTSHIFT */
    [0x58] = 55,   /* KEY_KEYPAD_MULTIPLY -> KEY_KPASTERISK */
    [0x59] = 56,   /* KEY_LEFT_ALT -> KEY_LEFTALT */
    [0x5A] = 57,   /* KEY_SPACE -> KEY_SPACE */
    [0x5B] = 58,   /* KEY_CAPS_LOCK -> KEY_CAPSLOCK */
...
static void ps2_keyboard_send_event(key_code_t key_code, key_state_t state, uint8* raw_scancode, uint8 length) {
    ...
    if (state == KEY_STATE_PRESSED || state == KEY_STATE_RELEASED) {
        ...
        // Deliver Ctrl+C/Ctrl+\ as SIGINT/SIGQUIT directly from IRQ context,
        // to the active tty's foreground process group. This is independent
        // of whether any task is currently blocked in a read() syscall --
        // readStr() (kb.c) only ever sees Ctrl+C if some task happens to be
        // actively reading a line at that exact moment, which is useless for
        // interrupting a job that's blocked elsewhere (e.g. a shell stuck in
        // waitpid() on a hung child). g_virtual_ttys[].fg_pgid is already
        // kept current by tcsetpgrp() (see syscall.c), so this reaches the
        // right target regardless of what any task is doing right now.
        if (event.ctrl && state == KEY_STATE_PRESSED &&
            (key_code == KEY_C || key_code == KEY_BACKSLASH)) {
            extern uint32_t g_current_tty_session;
            uint32_t vt = g_current_tty_session;
            debuglog(DEBUG_INFO, "[CTRLC] key_code=%u ctrl=%d vt=%u\n", key_code, event.ctrl, vt);
            if (vt >= 1 && vt <= MAX_VIRTUAL_TTYS) {
                virtual_tty_t* tty = &g_virtual_ttys[vt - 1];
                uint32 target_pgid = tty->fg_pgid;
                debuglog(DEBUG_INFO, "[CTRLC] tty_fd=%u fg_pgid=%u\n", vt - 1, target_pgid);
                if (target_pgid != 0) {
                    task_send_signal_to_pgrp(target_pgid,
                        key_code == KEY_C ? SIGINT : SIGQUIT);
                    debuglog(DEBUG_INFO, "[CTRLC] sent signal to pgrp %u\n", target_pgid);
                }
            }
        }
    }
    ...
    // Dispatch to input multiplexer (for hotkeys, desktop, etc.)
