 ...
 * Integrates with Fern SMP and interrupt management systems
 */
 ...
 *
 * Uses the PIT-based timer_sleep_ms() as a reference if the PIT timer has
 * already been initialised (timer_get_frequency() != 0).  When the PIT is not
 * yet running (early-boot call or deferred calibration) this function falls
 * back to a TSC-based busy-wait so it never blocks or hangs.
 *
 * The TSC fallback assumes a 1 GHz TSC, which is inaccurate on real hardware
 * but is sufficient for QEMU and gives a usable apic.timer_frequency until
 * a proper calibration can be done after the PIT is running.
 */
int apic_calibrate_timer(void)
{
    ...
    /* Sample LAPIC current count at the start */
    timer_start = apic_read_register(LAPIC_REG_TIMER_CURRENT);

    if (timer_get_frequency() != 0) {
        /*
         * PIT timer is running — use it as a reference.  This is the accurate
         * path and is taken when apic_calibrate_timer() is deferred until after
         * timer_init().
         */
        timer_sleep_ms(CALIBRATION_MS);
    } else {
        /*
         * PIT timer is not yet available.  Use a TSC busy-wait as a fallback.
         * Assumes ~1 GHz TSC — good enough to produce a ballpark figure.
         * 10 ms * 1e9 ticks/s = 10,000,000 ticks.
         */
        debuglog_printf("APIC: PIT not ready, using TSC fallback for calibration\n");
        uint64_t tsc_target = read_tsc() + (uint64_t)CALIBRATION_MS * 1000000ULL;
        while (read_tsc() < tsc_target) {
            __asm__ volatile("pause");
        }
    }

    /* Sample LAPIC current count at the end */
    timer_end = apic_read_register(LAPIC_REG_TIMER_CURRENT);

    /* Calculate timer frequency */
    uint32_t lapic_ticks = timer_start - timer_end;  /* counter counts down */

    if (lapic_ticks > 0) {
        apic.timer_frequency = (lapic_ticks * 1000) / CALIBRATION_MS;
        apic.timer_frequency *= 16;  /* Account for divisor of 16 */
        apic.timer_calibration = 1;
        debuglog_printf("APIC: Timer frequency calibrated to %u Hz\n",
                   apic.timer_frequency);
    } else {
        debuglog_printf("APIC: Timer calibration produced zero ticks, using 100 MHz fallback\n");
        apic.timer_frequency = 100000000;  /* Fallback: 100 MHz */
        apic.timer_calibration = 0;
    }

    /* Stop LAPIC timer — leave it masked until explicitly started */
    apic_write_register(LAPIC_REG_TIMER_LVT, APIC_LVT_MASKED);
 ...
 *
 * Safe to call on a single-CPU QEMU instance:
 *  - Returns -1 gracefully when no APIC is detected (QEMU PIC mode).
 *  - Never sends IPIs or waits for Application Processors.
 *  - Only disables the 8259A PIC after the APIC is confirmed operational,
 *    so that PIC-mode interrupt delivery remains intact on failure.
 *  - Timer calibration uses a TSC fallback when the PIT is not yet running.
 */
int local_apic_init(void)
{
    ...
    /* Detect APIC availability — returns false on PIC-only systems */
    if (!apic_detect_availability()) {
        debuglog_printf("APIC: Local APIC not available, remaining in PIC mode\n");
        return -1;
    }

    /* Detect x2APIC support (optional enhancement) */
    apic.x2apic_available = apic_detect_x2apic();

    /* Enable Local APIC in the MSR */
    ret = apic_enable_local_apic();
    if (ret != 0) {
        debuglog_printf("APIC: Failed to enable Local APIC, remaining in PIC mode\n");
        return ret;
    }
    ...
    /* Setup Local Vector Table entries */
    ...
    /*
     * Calibrate APIC timer.  Non-fatal: if calibration fails the frequency
     * falls back to a safe 100 MHz estimate.  Uses TSC busy-wait if the PIT
     * timer is not yet running (timer_get_frequency() == 0).
     */
    ret = apic_calibrate_timer();
    if (ret != 0) {
        debuglog_printf("APIC: Timer calibration failed (non-fatal)\n");
        /* Continue — apic_calibrate_timer already set a fallback frequency */
    }
    ...
    debuglog_printf("APIC: Local APIC initialization complete (single-CPU safe)\n");

    /*
     * Disable the legacy 8259A PIC only now that the APIC is fully operational.
     * Doing this earlier would leave the system without any interrupt controller
     * if APIC init had failed above.
     */
    pic_8259a_disable();
