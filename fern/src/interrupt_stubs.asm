...
isr_common_stub:
    ; Save original ESP in EBX so we can always access the raw interrupt
    ; frame independent of pushes below.
    ;
    ; On entry:
    ;   [esp + 0] = interrupt number
    ;   [esp + 4] = error code (real or synthetic)
    ;   [esp + 8] = CPU-pushed return frame (EIP, CS, EFLAGS, ...)
    mov ebx, esp
    ...
    ; Set up a stable frame pointer for debugging without mutating the
    ; CPU-pushed return frame values.
    ...
    lea ecx, [ebx + 8]          ; Pointer to CPU interrupt frame
