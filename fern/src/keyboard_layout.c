...
static const keyboard_layout_t keyboard_layout_us = {
    .normal = {
        ...
        // US keyboards have no separate ISO/102nd key, but if this layout
        // is ever selected on hardware that sends one anyway (as opposed to
        // the correct fix of selecting the gb layout), fall back to the
        // same character as the backslash key instead of silently emitting
        // nothing for whatever's physically in that position.
        [KEY_OEM_102] = '\\',
    ...
    .shifted = {
        ...
        [KEY_L] = 'L', [KEY_SEMICOLON] = ':', [KEY_APOSTROPHE] = '"',
        [KEY_GRAVE] = '~', [KEY_BACKSLASH] = '|',
        [KEY_OEM_102] = '|', // see .normal's KEY_OEM_102 comment above
        [KEY_Z] = 'Z', [KEY_X] = 'X', [KEY_C] = 'C', [KEY_V] = 'V',
        [KEY_B] = 'B', [KEY_N] = 'N', [KEY_M] = 'M', [KEY_COMMA] = '<',
        [KEY_PERIOD] = '>', [KEY_SLASH] = '?',
        [KEY_KEYPAD_0] = '0', [KEY_KEYPAD_1] = '1', [KEY_KEYPAD_2] = '2',
...
uint8 keyboard_layout_emit_chars(const keyboard_layout_t* layout,
    ...
    // Variable-length (not fixed-3-byte) so Home/End/Delete's 3-4 byte CSI
    // forms fit alongside the arrows' 3-byte ones; KEYBOARD_MAX_SEQUENCE_LENGTH
    // is 8, comfortably covering the longest entry here ("\x1b[3~", 4 bytes).
    // Only raw/non-canonical-mode readers (e.g. userspace/sh.c's line editor,
    // htop) ever see these bytes -- cooked mode (ICANON) intercepts the same
    // keys directly by key_code_t in tty_devices.c's ttyN_handle_keyboard_event()
    // before they'd reach here, moving edit_cursor itself instead.
    const struct {
        key_code_t key;
        const char *seq;
    } arrow_map[] = {
    ...
    for (size_t i = 0; i < sizeof(arrow_map) / sizeof(arrow_map[0]); ++i) {
        if (key_code == arrow_map[i].key) {
            uint8 seq_len = (uint8)strlen(arrow_map[i].seq);
            uint8 emit = (buffer_size < seq_len) ? buffer_size : seq_len;
    ...
    // Standard terminal convention: Ctrl+letter produces the control code
    // at that letter's position in the alphabet (Ctrl+A=0x01 ... Ctrl+Z=
    // 0x1A - e.g. Ctrl+C=0x03, the usual "interrupt" byte). Without this,
    // holding Ctrl had no effect on the character stream at all.
    if (ctrl) {
