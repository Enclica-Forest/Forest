...
/* ---- Public PC speaker fallback API (see include/sound_pcspeaker.h) ---- */

void snd_pcspeaker_tone(uint32_t freq, uint32_t ms) {
    if (freq == 0 || ms == 0) {
        return;
    }
    if (!g_pcspk_state.initialized) {
        SoundDriver* d = sound_pc_speaker_driver();
...
void snd_pcspeaker_beep(void) {
    snd_pcspeaker_tone(800, 120);
}

bool snd_pcspeaker_available(void) {
    return g_pcspk_state.initialized;
}
