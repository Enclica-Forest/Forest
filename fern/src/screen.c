...
static void safe_volatile_memmove(volatile uint8 *dest, const volatile uint8 *src, uint32 count) {
    ...
    /* VGA text memory is plain RAM (no side-effecting MMIO registers), so a
     * bulk word-sized copy is safe and much faster than a byte-at-a-time
     * volatile loop. The manual loop below was the actual cause of the
     * visible "scan down" scroll lag: every scroll copied every byte one at
     * a time with no vectorization (volatile blocks the compiler from
     * turning it into rep movs/SSE moves on its own). */
...
static inline void safe_tui_set_char(int x, int y, char c, int fg_color, int bg_color) {
    ...
    if (validate_screen_offset(pos + 1)) {
        vidmem[pos] = c;
        /* VGA attribute: fg (bits 0-3) | bg (bits 4-6).  Mask bg to 3 bits
         * so bit 7 (blink) is never accidentally set by background color. */
        vidmem[pos + 1] = ((bg_color & 0x7) << 4) | (fg_color & 0xF);
    }
}
