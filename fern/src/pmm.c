...
// Guards pmm_state (bitmap, free_frames, last_frame) so callers no longer
// need to hold interrupts off across allocation calls themselves.
...
#ifdef __x86_64__
    void* page = (void*)(uintptr_t)frame_to_addr(frame);
    memset(page, 0, MEMORY_PAGE_SIZE);
#else
    // frame_to_addr(frame) is a raw physical address, not necessarily a valid
    // kernel virtual pointer: once paging is on, only the boot-time identity
    // map (see vmm_set_identity_map_limit) is directly dereferenceable, and
    // frames from higher physical memory (routine once enough heap/user-heap
    // pages have been handed out) are not mapped anywhere. A direct memset()
    // there page-faults in kernel mode. Route through the temp-map window,
    // which is safe both before and after paging is enabled.
    uint32_t phys = frame_to_addr(frame);
    void* page = vmm_temp_map_page(phys);
    if (page) {
        memset(page, 0, MEMORY_PAGE_SIZE);
        vmm_temp_unmap_page(page);
    }
#endif
...
uint32_t pmm_alloc_frames(uint32 count) {
    ...
    void* block = (void*)(uintptr_t)frame_to_addr(start_frame);
    memset(block, 0, (size_t)count * MEMORY_PAGE_SIZE);
...
#ifdef __x86_64__
    uint64 scrubbed = 0;
    for (uint64 frame = pmm_state.kernel_end_frame + 1; frame < pmm_state.total_frames; frame++) {
#else
    uint32_t scrubbed = 0;
    for (uint32_t frame = pmm_state.kernel_end_frame + 1; frame < pmm_state.total_frames; frame++) {
#endif
        if (is_frame_used(frame)) {
            continue;
        }
        ...
        memset(page, 0, MEMORY_PAGE_SIZE);
#else
        // See pmm_alloc_frame(): frame_to_addr() is a raw physical address
        // and isn't safe to dereference directly once paging is enabled.
        uint32_t phys = frame_to_addr(frame);
        void* page = vmm_temp_map_page(phys);
        if (page) {
            memset(page, 0, MEMORY_PAGE_SIZE);
