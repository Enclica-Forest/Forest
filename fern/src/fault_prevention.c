...
static void fault_prevention_handle_page_fault(interrupt_context_t *context) {
    ...
    bool user             = (context->error_code & 0x04) != 0;  // CPL=3 at time of fault
    bool reserved         = (context->error_code & 0x08) != 0;
    (void)(context->error_code & 0x10);  // instruction_fetch: decoded but not used further

    if (reserved) {
        panic("Page fault due to reserved bit violation at 0x%lx", fault_address);
    }

    // Null pointer dereference: handle differently for user vs kernel mode.
    // Without SMEP/SMAP the CPU still enforces ring-level protection via CS.RPL,
    // so a user-mode null deref should kill the task, not corrupt kernel state.
    if (fault_address < 0x1000) {
        if (user) {
            // User-mode null pointer: terminate the offending task gracefully
            panic("User-mode null pointer dereference at 0x%lx - RIP: 0x%lx",
                  fault_address, context->rip);
        }
        // Kernel-mode null pointer: always panic - never skip the instruction
        panic("Kernel null pointer dereference at 0x%lx - RIP: 0x%lx",
              fault_address, context->rip);
    }

    // Demand-paging: only allocate pages for user-mode not-present faults
    ...
    // Kernel-mode fault: always panic (unrecoverable)
    if (!user) {
        panic("Kernel page fault at 0x%lx - RIP: 0x%lx, Error: 0x%lx",
              fault_address, context->rip, context->error_code);
    }

    // User-mode fault that demand-paging could not fix
    panic("User page fault at 0x%lx - RIP: 0x%lx, Error: 0x%lx",
          fault_address, context->rip, context->error_code);
}
...
fault_prevention_error_t fault_prevention_init(const fault_prevention_config_t *config) {
    ...
    interrupt_register_handler(8, fault_prevention_handle_double_fault, NULL);
    interrupt_register_handler(13, handle_general_protection_fault, NULL);
    interrupt_register_handler(14, fault_prevention_handle_page_fault, NULL);
