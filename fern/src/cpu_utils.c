...
uint32_t get_current_cpu_id(void) {
    /* BSP is CPU 0. On UP builds (or before SMP bring-up) this is correct.
     * On SMP, the APIC ID -> logical CPU index mapping is owned by smp.c;
     * callers that need the APIC ID should use the APIC layer directly. */
    return 0;
}
