/*
 * Comprehensive CPU Exception Handlers for Fern
...
/* Safely terminate the current userspace task after a fault.  If there is no
 * current task (e.g. fault during early boot) halt instead of corrupting the
 * scheduler state.  This function never returns. */
static __attribute__((noreturn)) void terminate_current_user_task(const char* reason)
{
    if (!current_task) {
        debuglog_printf("[EXCEPT] No current task for user fault (%s); halting\n",
                        reason ? reason : "unknown");
        cpu_disable_interrupts();
        for (;;) __asm__ __volatile__("hlt");
    }
    debuglog_printf("[EXCEPT] User task '%s' (PID %u) terminated: %s\n",
                    current_task->name, (unsigned)current_task->id,
                    reason ? reason : "unknown");
    task_exit(-1, reason ? reason : "userspace fault");
    /* task_exit must not return. */
    cpu_disable_interrupts();
    for (;;) __asm__ __volatile__("hlt");
}
...
static __attribute__((noreturn)) void panic_from_exception(int exception, struct interrupt_context *ctx, const char *reason);
...
void exception_dispatcher(struct interrupt_context *ctx)
{
    ...
    fatal = !exception_table[exception].recoverable || is_user_mode_exception(ctx);
    if (fatal) {
        debuglog_printf("Exception: Fatal exception %d (%s)\n",
                    exception, exception_table[exception].name);
        handle_fatal_exception(exception, ctx);
        return;
    }

    /* Unhandled recoverable kernel exception */
...
static irq_return_t handle_division_error(struct interrupt_context *ctx)
{
    ...
    if (is_user_mode_exception(ctx)) {
        terminate_current_user_task("floating point exception");
    }
    
    handle_fatal_exception(EXCEPTION_DIVIDE_ERROR, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_overflow(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_OVERFLOW, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_bound_range_exceeded(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_BOUND_RANGE_EXCEEDED, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_invalid_opcode(struct interrupt_context *ctx)
{
    ...
    if (is_user_mode_exception(ctx)) {
        terminate_current_user_task("illegal instruction");
    }

    handle_fatal_exception(EXCEPTION_INVALID_OPCODE, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_device_not_available(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_DEVICE_NOT_AVAILABLE, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_double_fault_internal(struct interrupt_context *ctx)
{
    handle_fatal_exception(EXCEPTION_DOUBLE_FAULT, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_segment_not_present(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_SEGMENT_NOT_PRESENT, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_stack_fault(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_STACK_FAULT, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_general_protection(struct interrupt_context *ctx)
{
    ...
    if (is_user_mode_exception(ctx)) {
        terminate_current_user_task("segmentation fault");
    }

    handle_fatal_exception(EXCEPTION_GENERAL_PROTECTION, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_page_fault_internal(struct interrupt_context *ctx)
{
    ...
    if (user && is_user_mode_exception(ctx)) {
        ...
        char reason[48];
        snprintf(reason, sizeof(reason), "segmentation fault @ 0x%x",
                 (uint32_t)fault_address);
        terminate_current_user_task(reason);
    }
    
    handle_fatal_exception(EXCEPTION_PAGE_FAULT, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_alignment_check(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_ALIGNMENT_CHECK, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_machine_check(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_MACHINE_CHECK, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_virtualization_exception(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_VIRTUALIZATION, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_control_protection(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_CONTROL_PROTECTION, ctx);
    return IRQ_NONE;
}
...
static irq_return_t handle_security_exception(struct interrupt_context *ctx)
{
    ...
    handle_fatal_exception(EXCEPTION_SECURITY, ctx);
    return IRQ_NONE;
}
 ...
 *
 * For user-mode exceptions we ALWAYS terminate only the offending userspace
 * task and never panic the whole kernel.  This is the cornerstone of robust
 * userspace error handling: a misbehaving app crashes itself, not the OS.
 */
static void handle_fatal_exception(int exception, struct interrupt_context *ctx)
{
    ...
    if (is_user_mode_exception(ctx)) {
        /* Terminate only the offending userspace task. */
        const char* reason = exception_table[exception].name;
        if (!reason || !reason[0]) {
            reason = "fatal userspace exception";
        }
        debuglog_printf("[EXCEPT] Terminating userspace task (PID %u) due to %s at EIP=0x%x\n",
                        current_task ? (unsigned)current_task->id : 0,
                        reason, (uint32_t)(uintptr_t)FRAME_IP(ctx->frame));

        /* If there is no current task (e.g. exception during boot before the
         * scheduler is up) we cannot safely terminate anything; halt instead
         * of corrupting the task list. */
        if (!current_task) {
            debuglog_printf("[EXCEPT] No current task during user-mode fault; halting\n");
            cpu_disable_interrupts();
            for (;;) __asm__ __volatile__("hlt");
        }
        ...
        /* task_exit() must not return.  If it ever does, halt safely rather
         * than falling through into the kernel panic path. */
        debuglog_printf("[EXCEPT] task_exit returned unexpectedly; halting\n");
        cpu_disable_interrupts();
        for (;;) __asm__ __volatile__("hlt");
    } else {
        const char *reason = exception_table[exception].name;
        if (!reason || !reason[0]) {
            reason = "Fatal exception in kernel mode";
        }
        panic_from_exception(exception, ctx, reason);
    }
}
...
static __attribute__((noreturn)) void panic_from_exception(int exception, struct interrupt_context *ctx, const char *reason)
{
    ...
    if (exception == EXCEPTION_PAGE_FAULT) {
        fault_addr = (uint32_t)cpu_read_cr2();
    }

    panic_preload_fault_info(
        fault_addr,
        (uint32_t)ctx->error_code,
        (uint32_t)FRAME_IP(ctx->frame),
        (uint32_t)ctx->frame.cs,
        (uint32_t)FRAME_FLAGS(ctx->frame));
