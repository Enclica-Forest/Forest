...
memory_result_t memory_init(uint32 multiboot_magic, uint32 multiboot_info) {
    ...
    if (memory_info.usable_memory_kb < 65536) {
        ...
        row++;
        VGA_PRINT(row++, "Fern cannot boot with this amount of RAM.");
        row++;
        ...
        print("\n");
        print("Fern cannot boot with this amount of RAM.\n");
        print("\n");
    ...
    // Never place the PMM bitmap/heap inside the kernel image. The default
    // MEMORY_PMM_START (~4MB) assumes a small kernel, but this kernel embeds
    // large read-only assets (~13MB of .data), pushing kernel_end well past
    // 4MB. If the bitmap/heap landed inside the image, PMM frame scrubbing
    // would zero the kernel's own .data (e.g. memory_info) — the kernel then
    // falsely reports 0 KB usable RAM and halts. Slide the base above the real
    // end of the loaded kernel.
    uint32 kernel_image_end = memory_align_up((uint32)&kernel_end, MEMORY_PAGE_SIZE);
    if (kernel_image_end > pmm_base) {
        debuglog(DEBUG_INFO,
                 "[MEM] Moving PMM/heap above kernel image: kernel_end 0x%08x, new PMM base 0x%08x\n",
                 (uint32)&kernel_end, kernel_image_end);
        pmm_base = kernel_image_end;
    }
    ...
    debuglog(DEBUG_INFO, "[MEM] Checking multiboot magic: 0x%08x (%s)\n", multiboot_magic,
             multiboot_magic == MULTIBOOT_BOOTLOADER_MAGIC  ? "Multiboot1 OK" :
             multiboot_magic == MULTIBOOT2_BOOTLOADER_MAGIC ? "Multiboot2 OK" :
             multiboot_magic == 0                           ? "no magic"      :
                                                             "unknown magic");
    ...
    print("[MEM] Scrubbing free physical frames...\n");
    ...
    // Tell vmm.c the real boundary of the boot-time identity map. Physical
    // frames at/above this address are NOT safe to treat as directly
    // dereferenceable kernel virtual pointers (see vmm_phys_pt_as_virt() in
    // vmm.c) even though they are still well below MEMORY_USER_START -
    // pmm_alloc_frame() can and does hand out frames from that higher range
    // (e.g. for user-space page tables) once enough physical memory has been
    // allocated.
    vmm_set_identity_map_limit(kernel_map_end);
