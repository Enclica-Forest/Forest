...
# QEMU_MEMORY is an integer MB value), after build/iso.mk (kernel/initrd defs)
# and after build/foreb.mk (so FOREBO_DIR and the forebo-image target exist).
#
...
#
# GRUB is gone: the x86 boot path now boots the foreboots (ForeB) disk image
# directly, so every QEMU network/USB/sound/KVM option assembled above still
# applies. forebo-image (build/foreb.mk) builds ForeB and embeds $(OUTPUT) as
# the Fern kernel; the raw images live under $(FOREBO_DIR)/build.
# Use recursive (`=`) so the media/$(OUTPUT) paths resolve at recipe time
# regardless of where this fragment is included relative to their definitions.
# ---------------------------------------------------------------------------
FOREBO_BIOS_IMAGE := $(FOREBO_DIR)/build/forebo.img   # raw-MBR (BIOS/hybrid) disk
FOREBO_ESP_IMAGE  := $(FOREBO_DIR)/build/esp.img      # FAT EFI System Partition

ifeq ($(filter arm aarch64,$(ARCH)),)
  ifeq ($(BOOT_MODE),uefi)
    QEMU_BOOT_MEDIA = -bios /usr/share/ovmf/OVMF.fd -drive format=raw,file=$(FOREBO_ESP_IMAGE)
    QEMU_RUN_DEPS   := forebo-image
  else
    # Boot the ForeB raw-MBR disk image (kernel embedded by forebo-image).
    QEMU_BOOT_MEDIA = -drive format=raw,file=$(FOREBO_BIOS_IMAGE)
    QEMU_RUN_DEPS   := forebo-image
  endif
