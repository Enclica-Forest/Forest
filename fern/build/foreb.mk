...
# =============================================================================
# ForeB (../foreboots/) is THE Forest-OS bootloader — the one and only boot
# path now that GRUB has been removed. It is the native raw-MBR/UEFI disk
# bootloader that loads the Fern kernel. Owns the `forebo*` phony targets
# (moved out of the main Makefile).
#
...
# foreboots lives as a SIBLING of the kernel dir (REPO_ROOT), one level up:
# REPO_ROOT = /home/bluet/forest/fern  ->  /home/bluet/forest/foreboots.
# $(abspath ...) normalizes the `..` so `$(MAKE) -C $(FOREBO_DIR)` gets a clean
# absolute path. Resolves correctly both before and after the dir rename.
FOREBO_DIR := $(abspath $(REPO_ROOT)/../foreboots)

# ForeB (Fern kernel) path follows the configured arch/boot/build output.
...
# relative $(OUTPUT) (e.g. build/32bit-bios-debug/boot/fern.bin) would resolve
# against foreboots/ in the sub-make and silently fail to embed the Fern kernel.
...
forebo:
	@echo "$(INFO_COLOR)Building ForeB - the Forest-OS bootloader...$(NO_COLOR)"
...
forebo-image: forebo
	...
	@if [ -f "$(FOREBO_KERNEL)" ]; then \
		...
		echo "$(WARN_COLOR)Fern kernel not found at $(FOREBO_KERNEL). Build the OS first.$(NO_COLOR)"; \
