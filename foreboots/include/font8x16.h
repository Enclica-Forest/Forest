/*
 * font8x16.h - Crisp 8x16 monochrome bitmap font for ForeB (UEFI UI).
 *
 * This is the classic IBM VGA 8x16 ROM text font (CP437 layout), the same
 * glyph set every PC BIOS text mode has used for decades. It is public
 * domain (a hardware ROM bitmap, not a creative work) and was transcribed
 * byte-for-byte from the Linux console "default8x16" VGA font. It provides
 * a full 256-glyph set: ASCII 0x20..0x7E are crisp text glyphs, and the
 * upper range includes the CP437 box-drawing / block characters
 * (0xB0..0xDF: shades, single/double lines, corners, full block 0xDB).
 *
 * This replaces the old 8x8 font (font8x8.h) in the UEFI renderer for a
 * much sharper, taller cell. Pair it with integer 2x scaling on hi-res GOP
 * framebuffers for a "10x better" look.
 *
 *   font8x16[glyph_index][row]
 *     glyph_index = c (0..255); a byte value directly indexes its glyph.
 *     row 0 = TOP scanline, row 15 = BOTTOM scanline.
 *
 * ============================================================================
 *  BIT ORDER - READ THIS BEFORE WRITING A RENDERER
 * ============================================================================
 *  This font is MSB-first (native VGA ROM order):
 *     bit 7 (mask 0x80) = LEFTMOST  pixel (column 0)
 *     bit 0 (mask 0x01) = RIGHTMOST pixel (column 7)
 *
 *  NOTE: this is the OPPOSITE bit order from font8x8.h (which is LSB-first).
 *  Do not blindly reuse the 8x8 inner loop. Correct rendering here:
 *
 *     for (row = 0; row < 16; row++) {
 *         unsigned char bits = font8x16[glyph][row];
 *         for (col = 0; col < 8; col++)
 *             if (bits & (0x80u >> col))
 *                 put_pixel(x + col, y + row, fg);   // or fill_rect for scale
 *     }
 *
 *  Cell is 8 wide x 16 tall. Advance is exactly 8 px per character (times
 *  the integer scale). Most glyph ink sits in the upper-left, leaving the
 *  bottom rows as descender / line spacing.
 * ============================================================================
 */
#ifndef FOREB_FONT8X16_H
#define FOREB_FONT8X16_H

#define FONT8X16_W          8
#define FONT8X16_H          16
#define FONT8X16_COUNT      256   /* full 0x00..0xFF CP437 glyph set */

/*
 * font8x16_index(c): a byte directly indexes its glyph, so this is just a
 * clamp to 0..255. Provided for symmetry with font8x8_index() so renderer
 * code reads the same in both fonts.
 */
static inline int font8x16_index(int c)
{
    return (int)((unsigned char)c);
}

/* [glyph][row]; MSB (0x80) = leftmost pixel. See header comment. */
